...
public class LevelManager : Singleton<LevelManager>
{
    ...
    // ONLY FOR TEST
    public Point randomPoint { get; private set; }


    // ONLY FOR TEST
    public Point randomPointDest { get; private set; }
    ...
    // ONLY FOR TEST
    public GameObject RandomGameObject { get; private set; }

    // ONLY FOR TEST
    public GameObject RandomGameObjectDest { get; private set; }
    ...
    public Stack<Node> MonsterPath
    {
        get
        {
            if (monsterPath == null)
            {
                GeneratePath(randomPoint,randomPointDest);
            }

            if (monsterPath != null)
                return new Stack<Node>(new Stack<Node>(monsterPath));
    ...
    protected virtual void SpawnPortals()
    {
        ...
        randomPoint = new Point(0, 6);
        RandomGameObject = (GameObject)Instantiate(bluePortalPrefab, Tiles[randomPoint].GetComponent<TileScript>().WorldPosition, Quaternion.identity);

        randomPointDest = new Point(8, 2);
        RandomGameObjectDest = (GameObject)Instantiate(bluePortalPrefab, Tiles[randomPointDest].GetComponent<TileScript>().WorldPosition, Quaternion.identity);
    ...
    public void GeneratePath(Point source, Point destination)
    {
        ...
        //Point monsterPos = randomPoint;
        //Point monsterDest = new Point();

        monsterPath = AStar.GetPath(source, destination);
